...
#EXACT SAME PLOTS AS THE ALL STAR PLOTS for consistency in visualziations
#only difference is filtered by all star = 0

# Filter for All-Stars
non_all_stars_df <- df %>%
  filter(was_all_star == 0)

# Histogram for Points Per Game (PTSpg)
hist_pts <- ggplot(non_all_stars_df, aes(x = PTSpg)) +
...
hist_trb <- ggplot(non_all_stars_df, aes(x = TRBpg)) +
...
hist_ast <- ggplot(non_all_stars_df, aes(x = ASTpg)) +
...
hist_mpg <- ggplot(non_all_stars_df, aes(x = MPpg)) +
...
# Histogram for Age
hist_age <- ggplot(filter(all_stars_df, !is.na(Age)), aes(x = Age)) +
  geom_histogram(aes(y = ..density..), binwidth = 1, fill = "purple", color = "black", alpha = 0.7) +
  geom_density(alpha = .2, fill = "#FF6666") +
  ggtitle("Age of Non All-Star Rookies") +
  xlab("Age") +
  ylab("Density") +
  theme_minimal()
...
combined_histograms <- (hist_pts | hist_trb) /
  ...
  plot_annotation(title = 'Distributions of Key Statistics for Non All-Star Rookies (1980-2020)',
                  theme = theme(plot.title = element_text(hjust = 0.5, size = 16)))

combined_histograms
# Save the combined histograms plot
#ggsave("images/all_star_rookie_histograms_R.png", plot = combined_histograms, width = 12, height = 15)



# Scatter Plot for PTSpg vs. MPpg
scatter_pts_mpg <- ggplot(non_all_stars_df, aes(x = MPpg, y = PTSpg)) +
...
non_all_stars_age_pts <- non_all_stars_df %>%
  filter(!is.na(Age) & !is.na(PTSpg))

scatter_age_pts <- ggplot(non_all_stars_age_pts, aes(x = Age, y = PTSpg)) +
...
combined_scatterplots <- combined_scatterplots +
  plot_annotation(title = 'Relationships Between Key Statistics for Non All-Star Rookies (1980-2020)',
                  theme = theme(plot.title = element_text(hjust = 0.5, size = 16)))
combined_scatterplots
combined_histograms

# Save the combined scatter plots
#ggsave("images/all_star_rookie_scatter_plots_R.png", plot = combined_scatterplots, width = 14, height = 6)
