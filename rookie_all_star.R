...
hist_pts <- ggplot(all_stars_df, aes(x = PTSpg)) +
...
hist_trb <- ggplot(all_stars_df, aes(x = TRBpg)) +
...
hist_ast <- ggplot(all_stars_df, aes(x = ASTpg)) +
...
hist_mpg <- ggplot(all_stars_df, aes(x = MPpg)) +
...
hist_age <- ggplot(filter(all_stars_df, !is.na(Age)), aes(x = Age)) +
  ...
  geom_density(alpha = .2, fill = "#FF6666") +
  ggtitle("Age of All-Star Rookies") +
  xlab("Age") +
  ylab("Density") +
  theme_minimal()
...
combined_histograms <- (hist_pts | hist_trb) /
  ...
  plot_annotation(title = 'Distributions of Key Statistics for All-Star Rookies (1980-2020)',
...
scatter_pts_mpg <- ggplot(all_stars_df, aes(x = MPpg, y = PTSpg)) +
...
all_stars_age_pts <- all_stars_df %>%
  filter(!is.na(Age) & !is.na(PTSpg))

scatter_age_pts <- ggplot(all_stars_age_pts, aes(x = Age, y = PTSpg)) +
...
combined_scatterplots <- combined_scatterplots +
  plot_annotation(title = 'Relationships Between Key Statistics for All-Star Rookies (1980-2020)',
                  theme = theme(plot.title = element_text(hjust = 0.5, size = 16)))
